...
describe("platform health checks", () => {
  it("reports database success", async () => {
    const result = await getPlatformHealth({ queryFn: vi.fn().mockResolvedValue({ rows: [] }), storageCheck: healthyStorage });
  ...
  it("reports unavailable when the database check fails without exposing the raw error", async () => {
    const result = await getPlatformHealth({ queryFn: vi.fn().mockRejectedValue(new Error("password secret-db-host")), storageCheck: healthyStorage });
  ...
  it("makes the overall platform unavailable when required storage fails", async () => {
    const storageCheck = async () => ({ status: "unavailable", message: "Required storage is unavailable", directories: [] });
    const result = await getPlatformHealth({ queryFn: vi.fn().mockResolvedValue({ rows: [] }), storageCheck });
    expect(result.status).toBe("unavailable");
  });
});
