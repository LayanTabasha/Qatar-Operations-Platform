...
describe("frontend Archive workflow", () => {
  const sources = () => ({
    ...
    settings: ["frontend/pages/settings/settings-shared.js", "frontend/pages/settings/settings-page.js"].map(readRootFile).join("\n"), sites: readRootFile("frontend/pages/sites/sites-data.js"),
  ...
  it("loads every changed browser asset with a current cache token", () => {
    const { index } = sources();
    expect(index).toContain("app.js?v=20260825-related-fault-selector-v3");
    ...
    expect(index).toContain("js/state.js?v=20260827-request-fault-link-v1");
    ...
    expect(index).toContain("frontend/shared/modals/modal-submit.js?v=20260825-fault-visit-ux-v1");
    expect(index).toContain("styles.css?v=20260830-platform-health-alert-v1");
  });
});
