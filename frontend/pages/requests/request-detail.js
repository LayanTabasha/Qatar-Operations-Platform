...
function requestRelatedFaultMarkup(item) {
  const fault = item.related_fault;
  if (!fault) return `<section class="request-detail-section full"><h3>Related Fault</h3><p class="request-muted">No related Fault linked.</p></section>`;
  return `<section class="request-detail-section request-related-fault full"><h3>Related Fault</h3><article class="request-selected-fault"><header><div><strong>${requestText(fault.fault_reference)} • ${requestText(fault.title)}</strong><small>${requestText(fault.site_name)}${fault.charger_name ? ` • ${requestText(fault.charger_name)}` : ""}</small></div><span class="request-pill status-${requestText(fault.status)}">${requestText(requestLabel(fault.status))}</span></header><button class="secondary-button" data-fault-detail="${requestText(fault.id)}" type="button">View Fault</button></article><div class="request-linked-visits"><div><h4>Linked Site Visits</h4><small>From the related Fault</small></div>${(fault.related_site_visits || []).length ? fault.related_site_visits.map((visit) => `<article><div><strong>${requestText(formatMediumDate(visit.visit_date))}${visit.time_in || visit.time_out ? ` • ${requestText(visit.time_in || "—")}–${requestText(visit.time_out || "—")}` : ""}</strong><span>${requestText(visit.purpose || "Site Visit")}</span><small>${requestText(requestLabel(visit.status || ""))}${visit.engineer ? ` • ${requestText(visit.engineer)}` : ""}</small></div><button class="secondary-button" data-visit-detail="${requestText(visit.id)}" type="button">View Visit</button></article>`).join("") : `<p class="request-muted">No Site Visits linked to this Fault yet.</p>`}</div></section>`;
}
...
async function openRequestDetails(id) {
  ...
  try {
    ...
    form.dataset.requestMode = isRequestResponder() ? "hq-edit" : (canEditRequestStatus() ? "status-edit" : "details");
